...
export default function ConfigurationSelector() {
  ...
  function handlePrintView(newValue: boolean) {
    setConfiguration({ ...configuration, printView: newValue });
  }
  ...
  return (
    <div className="flex flex-row print:hidden">
      <NumberInput
        ...
        handler={handleSlots}
        step={1}
        value={configuration.slotsPerPage}
        label="Slots per binder page"
      />
      <ConfigurationToggle
        label="Show colors"
        handle={handleColors}
        checked={!!configuration.showColors}
      />
      <ConfigurationToggle
        label="Show Links"
        handle={handleLinks}
        checked={!!configuration.showLinks}
      />
      <ConfigurationToggle
        label="Show Supertypes"
        handle={handleTypes}
        checked={!!configuration.showTypes}
      />
      <ConfigurationToggle
        label="Print view"
        handle={handlePrintView}
        checked={!!configuration.printView}
      />
    </div>
  );
}
